...
/*JCV PARA BORRAR*/
$(document).on('click', '#delete_cuentasatributos', function(e){
...
function newCuentasatributos()
 {
     
    openCuentasatributo('nuevo',null,null);
    $('#modal_iconified').modal('show');
 }
function openCuentasatributo(action, id_atributo, codigo_atributo, nombre, estado)
 {
    ...
    $('#modal_iconified').on('shown.bs.modal', function () {
     ...
     if (action == 'nuevo'){
      ...
      $('#txtID').val(id_atributo);
      $('#txtCodigo').val(codigo_atributo);
...
function enviar_frm()
{
  var urlprocess = 'web/ajax/ajxcuentasatributos.php';
  var proceso = $("#txtProceso").val();
  var id_atributo = $("#txtID").val();
  var nombre =$("#txtNombre").val();
  var estado = $('#chkEstado').is(':checked') ? 1 : 0;
  

  var dataString='proceso='+proceso+'&id_atributo='+id_atributo+'&estado='+estado;
     ...
     success: function(data){
        ...
        if(data=="Validado"){
             ...
             if(proceso=="Registro"){
              ...
              cargarDiv("#reload-div","web/ajax/reload-cuentasatributos.php");
              limpiarform();
...
function Print_Report(Criterio)
{
  ...
  if(Criterio == 'Activos')
  {
       window.open('reportes/Cuentasatributos_Activos.php',
      'win2',
      ...
      'fullscreen=yes');
       ...
       window.open('reportes/Cuentasatributos_Inactivos.php',
      'win2',
      'status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
            ...
            function(isConfirm){
                if (isConfirm) {
                     return new Promise(function(resolve) {
                        $.ajax({
                        url: 'web/ajax/ajxcuentasatributos.php',
                        type: 'POST',
                        ...
                        .done(function(response){
                         swal('Borrado!', response.message, response.status);
                          ...
                          cargarDiv("#reload-div","web/ajax/reload-cuentasatributos.php"); // JCV PARA QUE RECARGUE LOS DATOS ACTUALIZADOS
                        })
                        .fail(function(){
