...
function newCliente()
 {
    ...
    openCliente('nuevo',null,null,null,null,null,null,null,null);
    $('#modal_iconified').modal('show');
 }
function openCliente(action, id_cuentas_de_bancos, codigo_cliente, fecha, concepto, ingreso, egreso, saldo, observaciones, tipo_de_gasto, estado)
 {
    ...
    $('#modal_iconified').on('shown.bs.modal', function () {
     ...
     /* $('#txtDireccion').prop( "disabled" , false);*/
      $('#txtTipo_de_gasto').prop("disabled",false);
      ...
      $('#txtID').val(id_cuentas_de_bancos);
     ...
     /* $('#txtDireccion').val(direccion);*/
      ...
      $('#txtTipo_de_gasto').val(tipo_de_gasto);
     ...
     /* $('#txtLimitC').prop( "disabled" , true);*/
      ...
      $('#txtTipo_de_gasto').prop("disabled",true);
      $('#chkEstado').prop( "disabled" , true);
...
function enviar_frm()
{
  var urlprocess = 'web/ajax/ajxcuentasdebancosJCV.php';
  var proceso = $("#txtProceso").val();
  var id_cuentas_de_bancos = $("#txtID").val();
  var fecha =$("#txtF1").val();
  ...
  var tipo_de_gasto = $('#txtTipo_de_gasto').val();

  var dataString='proceso='+proceso+'&id_cuentas_de_bancos='+id_cuentas_de_bancos+'&fecha='+fecha+'&ingreso='+ingreso+'&egreso='+egreso+'&estado='+estado;
  dataString+='&concepto='+concepto+'&saldo='+saldo+'&tipo_de_gasto='+tipo_de_gasto+'&observaciones='+observaciones;
//alert('holaenviarfrm');
  $.ajax({
     type:'POST',
     url:urlprocess,
     ...
     success: function(data){
        ...
        if(data=="Validado"){
             ...
             if(proceso=="Registro"){
              ...
              cargarDiv("#reload-div","web/ajax/reload-cuentasdebancosJCV.php");
              limpiarform();
...
function Print_Report(Criterio)
{
  ...
  if(Criterio == 'Activos')
  {
       window.open('reportes/Cuentasdebancos_ActivosJCV.php',
      'win2',
      ...
      'fullscreen=yes');
       ...
       window.open('reportes/Cuentasdebancos_InactivosJCV.php',
      'win2',
      'status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
            ...
            function(isConfirm){
                if (isConfirm) {
                     return new Promise(function(resolve) {
                        $.ajax({
                        url: 'web/ajax/ajxcuentasdebancosJCV.php',
                        type: 'POST',
                        ...
                        .done(function(response){
                         swal('Borrada!', response.message, response.status);
                          ...
                          cargarDiv("#reload-div","web/ajax/reload-cuentasdebancosJCV.php"); // JCV PARA QUE RECARGUE LOS DATOS ACTUALIZADOS
                        })
                        .fail(function(){
