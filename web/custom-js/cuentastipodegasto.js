...
/*JCV PARA BORRAR*/
$(document).on('click', '#delete_cuentastipodegasto', function(e){
...
function newCuentastipodegasto()
 {
     
    openCuentastipodegasto('nuevo',null,null);
    $('#modal_iconified').modal('show');
 }
function openCuentastipodegasto(action, id_tipodegasto, codigo_tipodegasto, nombre, estado)
 {
    ...
    $('#modal_iconified').on('shown.bs.modal', function () {
     ...
     if (action == 'nuevo'){
      ...
      modal.find('.title-form').text('Ingresar Atributo');
     }else if(action=='editar') {

      $('#modal_iconified').modal('show');

      $('#txtProceso').val('Edicion');
      $('#txtID').val(id_tipodegasto);
      $('#txtCodigo').val(codigo_tipodegasto);
      ...
      modal.find('.title-form').text('Actualizar Tipo de gasto');
     } else if(action=='ver'){
      $('#txtProceso').val('');
      $('#txtID').val(id_tipodegasto);
      $('#txtNombre').val(nombre);
      ...
      modal.find('.title-form').text('Ver Tipo de gasto');
     }
...
function enviar_frm()
{
  var urlprocess = 'web/ajax/ajxcuentastipodegasto.php';
  var proceso = $("#txtProceso").val();
  var id_tipodegasto = $("#txtID").val();
  var nombre =$("#txtNombre").val();
  var estado = $('#chkEstado').is(':checked') ? 1 : 0;
  

  var dataString='proceso='+proceso+'&id_tipodegasto='+id_tipodegasto+'&estado='+estado;
     ...
     success: function(data){
        ...
        if(data=="Validado"){
             ...
             if(proceso=="Registro"){
              ...
              cargarDiv("#reload-div","web/ajax/reload-cuentastipodegasto.php");
              limpiarform();
...
function Print_Report(Criterio)
{
  ...
  if(Criterio == 'Activos')
  {
       window.open('reportes/Cuentastipodegasto_Activos.php',
      'win2',
      ...
      'fullscreen=yes');
       ...
       window.open('reportes/Cuentastipodegasto_Inactivos.php',
      'win2',
      'status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
...
function SwalDelete(productId){
              swal({
                title: "¿Está seguro que desea borrar el atributo?",
            ...
            function(isConfirm){
                if (isConfirm) {
                     return new Promise(function(resolve) {
                        $.ajax({
                        url: 'web/ajax/ajxcuentastipodegasto.php',
                        type: 'POST',
                        ...
                        .done(function(response){
                         swal('Borrado!', response.message, response.status);
                          ...
                          cargarDiv("#reload-div","web/ajax/reload-cuentastipodegasto.php"); // JCV PARA QUE RECARGUE LOS DATOS ACTUALIZADOS
                        })
                        .fail(function(){
