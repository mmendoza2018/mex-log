$(document).ready(function() {   
    ...
    $("#resultados").load("./vistas/ajax/agregar_tmp_cotOKJCVOK.php");
    $("#f_resultado").load("./vistas/ajax/incrementa_fact_cotOKJCVOK.php");
    $("#datos_factura").load(); 
    $("#barcode").focus();
    load(1);
...
function load(page) {
    ...
    $("#loader").fadeIn('slow'); 
    $.ajax({ 
        url: './vistas/ajax/productos_modal_ventasOKJCVOK.php?action=ajax&page=' + page + '&q=' + q,
...
function agregar(id) {
    ...
    $.ajax({
        type: "POST", 
        url: "./vistas/ajax/agregar_tmp_modalcotOKJCVOK.php", 
...
$("#barcode_form").submit(function(event) {
    ...
    $.ajax({
        type: "POST",
        url: "./vistas/ajax/agregar_tmp_cotOKJCVOK.php", 
        data: parametros,
        beforeSend: function(objeto) {
            $("#resultados").html('<img src="./img/ajax-loaderOKJCV.gif"> Cargando...');
        },
        success: function(datos) {
            ...
            $("#id").focus();
            $("#barcode").val("");
            $("#f_resultado").load("./vistas/ajax/incrementa_fact_cotOKJCVOK.php"); //Actualizamos el numero de Factura
        }
    });
...
$("#datos_factura").submit(function(event) {
    ...
    $.ajax({
        type: "POST",
        url: "./vistas/ajax/guardar_cotizacionOKJCVOK.php",  
        data: parametros,
        beforeSend: function(objeto) {
        ...
        success: function(datos) {
            ...
            $("#resultados").load("./vistas/ajax/agregar_tmp_cotOKJCVOK.php"); // carga los datos nuevamente
            $("#f_resultado").load("./vistas/ajax/incrementa_fact_cotOKJCVOK.php"); // carga la caja de incrementar la factura
            $("#barcode").focus();
            load(1);
...
function imprimir_factura(user_id) {
    VentanaCentrada('../pdf/documentos/corte_cajaOKJCV.php?user_id=' + user_id, 'Corte', '', '724', '568', 'true');
}