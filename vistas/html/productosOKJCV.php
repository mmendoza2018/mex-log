<?php
/* jcv lo quito PARA EL MENU DE ADMIN
...
<?php require './vistas/html/includes/header_endOKJCV0.php';?>  <!--JCV CON EL ARCHIVO header_endOKJCV0.php MODIFIQUE LA VISTA ME GUSTÓ MÁS EL ARCHIVO header_endOKJCV.php ES EL ORIGINAL-->
...
if ($permisos_editar == 1) {
    //include "./vistas/modal/buscar_productos_ventasJCV.php";
        ...
        include "./vistas/modal/editar_productoOKJCV.php";
        include "./vistas/modal/eliminar_productoOKJCV.php";
    }
    ?> 
												...
												<div class="col-md-2">
													<div class="btn-group pull-right">
														 <button type="button" class="btn btn-success btn-rounded waves-effect waves-light" data-toggle="modal" data-target="#nuevoProducto"><i class="fa fa-plus"></i> Agregar</button>
                                                                                                                <!--<button type="button" class="btn btn-success btn-rounded waves-effect waves-light" data-toggle="modal" data-target="#buscar"><i class="fa fa-plus"></i> Agregar</button>-->
													</div>
...
<script type="text/javascript" src="./js/VentanaCentradaOKJCV.js"></script>
	<script type="text/javascript" src="./js/productosOKJCV.js"></script>
	<script>
		function precio_venta(){
			...
			$.ajax({
				...
				type:"POST",
				//url:'../ajax/precio.php',
                                url:'./vistas/ajax/precioOKJCV.php',
				data: parametros,
	...
	<script>
		function upload_image(product_id){
			...
			$.ajax({
					url: "./vistas/ajax/imagen_product_ajax.php",        // Url to which the request is send
		...
		function upload_image_mod(id_producto){
			...
			$.ajax({
					//url: "../ajax/imagen_product_ajax2.php",        // Url to which the request is send
                                        url: "./vistas/ajax/imagen_product_ajax2.php",        // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data: data, 			  // Data sent to server, a set of key/value pairs (i.e. form fields and values)
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
	...
	<script>
		function carga_img(id_producto) {
			$(".outer_img").load("./vistas/ajax/img.php?id_producto=" + id_producto);
		}
		function reporte_excel(){
			var q=$("#q").val();
			window.location.replace("./vistas/excel/rep_productos.php?q="+q);
...
function reporte(){
		...
		VentanaCentrada('./vistas/pdf/documentos/rep_productos.php?daterange='+daterange+"&categoria="+categoria,'Reporte','','800','600','true');
	}
</script>
