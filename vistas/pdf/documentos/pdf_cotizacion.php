...
*/
require '../../../pdfsjcv/vendor/autoload.php';
...
// JCV ORIGINAL PERO LO CAMBIE  nombre _factura desde buscar_cotizacionOKJCV.php $id_factura = intval($_GET['id_factura']);

$id_factura = $_GET['id_factura'];
$sql_count  = mysqli_query($conexion, "select * from facturas_cot where numero_factura='" . $id_factura . "'");
$count      = mysqli_num_rows($sql_count);
if ($count == 0) {
    echo "<script>alert('Cotización no encontrada')</script>";
    echo "<script>window.close();</script>";
    exit; 
} 
$sql_factura    = mysqli_query($conexion, "select * from facturas_cot where numero_factura='" . $id_factura . "'");
...
ob_start();
include dirname(__FILE__) . '/res/pdf_cotizacion_html.php';
...
require dirname(__FILE__) . '/res/pdf_cotizacion_html.php';  
