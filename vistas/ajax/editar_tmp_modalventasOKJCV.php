...
//Cmprobamos si agregamos un producto a la tabla tmp_compra
    ...
    if ($row = mysqli_fetch_array($comprobar)) {
        $cant = $row['cantidad'] + $cantidad;
         $importe_venta = $cant * $precio_venta;
        ...
        if ($cant > $stock and $inv == 0) {
            ...
            $sql          = "UPDATE detalle_fact_ventas SET cantidad='" . $cant . "', importe_venta='" . $importe_venta . "' WHERE id_producto='" . $id . "' and id_factura='" . $id_factura . "'";
...
// condicion si el stock e menor que la cantidad requerida 
        if ($cantidad > $stock and $inv == 0) {
            ...
            $importe_venta = $cantidad * $precio_venta;
            $insert_tmp = mysqli_query($conexion, "INSERT INTO detalle_fact_ventas (id_factura,numero_factura, id_producto,cantidad,precio_venta,importe_venta,origen) VALUES ('$id_factura','$numero_factura','$id','$cantidad','$precio_venta', '$importe_venta', '$numero_factura')");
            $update     = eliminar_stock($id, $cantidad); // Descuenta del inventario
        }
